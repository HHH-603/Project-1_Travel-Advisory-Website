  ...
  $.ajax({
    ...
    var countryList = Object.keys(response.data).map(function (code) {
      return {
        ...
        advisory: response.data[code].advisory,
      };
    });
    ...
    var queryURL =
      ...
      countryInput +
      "?yesterday=true&twoDaysAgo=false&strict=false&allowNull=true";
    $.ajax({
      url: queryURL,
      method: "GET",
    }).then(function (response) {
      ...
      $("#covidResults")
        .append("Cases per million:  ")
        .append(response.casesPerOneMillion);
      $("#covidResults")
        .append("Deaths per million:  ")
        .append(response.deathsPerOneMillion);
...
function nytAPI() {
  ...
  qParams.q = cityInput
    .val()
    .trim()
    .concat(" ", countryInput.val().trim(), " covid");
...
function updateNewsSection(response) {
  ...
  for (var i = 0; i < numArticles; i++) {
    ...
    articleDiv.append(
      $("<p>").text(
        "Date: " + moment(currentArticle.pub_date).format("MMM Do YYYY")
      )
    );
